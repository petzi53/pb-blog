...
write_coins <- function(path='') {
    ...
    # HERE IS CHANGE NECESSARY (2021-05-16)
    # slug has precedence!
    # collect from coinsYaml$coinsSlug (e.g., "using-emojis-in-r-and-rmarkdown")
    # and coinsURL (e.g., "post/2021/2021-04-19-using-emojis-in-r-and-rmarkdown.en/using-emojis-in-r-and-rmarkdown")
...
combine_yaml <- function(yfrontmatter, yconfig) {
    ...
    coinsYamlList <- list(coinsDoctitle = yfrontmatter$title,
                           ...
                           coinsSummary = yfrontmatter$summary,
...
coins_frontmatter <- function(docText, configText, coinsAll, fPath) {
    ...
    #+++++++++++++++++++++++++++++++++++++++++++++++
    # warn if document has no summary (abstract)
    if (is.null(yml[['summary']])) {
        warning(paste(fPath, "has no 'summary' in yaml frontmatter."), call. = FALSE)
        yml[['summary']] = ''
    }
    return(yml)
}
...
build_coins_string <- function(coinsURL, ylist, sep=" :: ") {
    ...
    "<span class='Z3988' title='url_ver=Z39.88-2004&amp;ctx_ver=Z39.88-2004&amp;rfr_id=info%3Asid%2Fzotero.org%3A2&amp;rft_val_fmt=info%3Aofi%2Ffmt%3Akev%3Amtx%3Adc", a,
         "rft.type=",      'blogPost', a,
         "rft.title=",      URLencode(ylist[['coinsDoctitle']]), a,
         "rft.source=",     URLencode(ylist[['coinsBlogtitle']]), a,
         "rft.rights=",     URLencode(ylist[['coinsCopyright']]), a,
         "rft.description=",URLencode(ylist[['coinsSummary']]), a,
